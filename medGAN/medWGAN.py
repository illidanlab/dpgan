...
def c2b(train, generated, adj=1.0):
    '''Set the number of 1 in generated data as multiple time of in training data, the rest is set to 0 (or not)'''

    if np.count_nonzero(generated) <= np.count_nonzero(train): # special case: number of 1 in generated is <= train, all nonzero in train = 1
    ...
    idx = int(np.around(adj*p*len(g))) # with adjustment
    v = g[idx] # any value large than this set to 1, o.w. to 0
    np.putmask(generated, generated<=v, 0.0) # due to the property of putmask, must first set 0 then set 1
    ...
    print float(np.count_nonzero(generated))/generated.size
    return generated
